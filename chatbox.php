...
$qur1=mysql_query("select * from chatbox where userid='".$_SESSION['user']."'");
  
$qur2=mysql_query("select * from accept where uId='".$_SESSION['user']."' ");
$qur3=mysql_query("select * from accept where sId='".$_SESSION['user']."' ");

$qur4  =mysql_query("select * from profile where userid='".$_SESSION['user']."'");
$image=mysql_fetch_array($qur4);
...
.tab{    background:url(images/bg.jpg);}
  #heading{font-size:25px;font-family:Arial, Helvetica, sans-serif;;color:#595959;}
   #msz{font-size:18px;font-family:Arial, Helvetica;margin-left: 20px;
 }
...
function fieldcheck(field,alerttext)
{
 	...
 	function studentformcheck(thisform)
 {
  with(thisform)
   {
   if((fieldcheck(friends,"Select your friends")==false)||
   (fieldcheck(sendmsz,"type the message")==false))
  ...
  <div class="main_resize">
    <div class="header">
      ...
      <div class="search">
          ...
          <span>
          <input type="text"  name="searchchat" id="s" />
     <input name="searchsubmit" type="image" src="images/search.gif" id="searchsubmit" class="btn"  />
      ...
      <div class="hbg">
          \
         <div class="tab">
              <form name="msz" method="post" action="chatcode.php" onSubmit="return studentformcheck(this)">
           <center> <label id="heading" >Chat Box</label></center><br><br><br>
             ...
             <div id="msz">
              ...
              <label > Select Friends 
                 ...
                 <option value=<?echo $row2['sFirst'];?>><?echo $row2['sFirst'];?></option>
                  <?}?>
                  
                  <? while($row3=mysql_fetch_array($qur3)){?>
                    <option value=<?echo $row3['uFirst'];?>><?echo $row3['uFirst'];?></option>
                  <?}?>
                  </select>
              </label> 
              <br> <hr> 
              <label >Send Message</label><br>
              <label><textarea name="sendmsz" rows="4" cols="35"></textarea></label>
                <label> <input type="submit" value="send" name="sub"></label> 
              <br>
              <hr>
              <label>Message</label><br>
              <label><textarea name="msz" rows="10" cols="35"><? while($m=mysql_fetch_array($qur1)){ echo $m['ufirst']."  :".$m['sendmsz'].PHP_EOL;}?></textarea></label>
              <br><br><br>
